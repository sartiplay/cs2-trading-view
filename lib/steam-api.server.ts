...
export async function fetchSteamPrice(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[Steam API] HTTP ${response.status} for ${marketHashName}`
      );
    ...
    if (data.success && data.median_price) {
      ...
      console.log(
        `[Steam API] Successfully fetched price for ${marketHashName}: $${price.toFixed(
          2
        )}`
      );
      return price;
    ...
    if (error instanceof Error) {
      console.error(
        `[Steam API] Failed to fetch price for ${marketHashName}: ${error.message}`
      );
    } else {
...
export async function fetchMultiplePrices(
  ...
  Array<{ market_hash_name: string; price: number | null; error?: string }>
> {
  const results = [];
